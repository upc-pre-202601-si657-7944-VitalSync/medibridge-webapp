...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, SharedI18nModule],
  template: `
    <div class="page-container">
      ...
      @if (patient()) {
        ...
        <form [formGroup]="linkForm" (ngSubmit)="onLinkPatient()" class="link-form">
          ...
          <input id="patientId" formControlName="patientId" />
  ...
  styles: [`
    ...
    .patient-card { background: white; padding: 2rem; border-radius: 1rem; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
    .link-form { display: grid; gap: 0.75rem; max-width: 22rem; }
    input { width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 0.5rem; }
    button { width: fit-content; padding: 0.75rem 1rem; background: #2563eb; color: white; border: none; border-radius: 0.75rem; cursor: pointer; }
  `]
})
